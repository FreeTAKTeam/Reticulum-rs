...
fn python_accepts_rust_announces_gate() {
    ...
    let announce_packet = destination
        .announce(OsRng, None)
        .expect("rust announce packet");
    let path_response_packet = destination
        .path_response(OsRng, None)
        .expect("rust path response packet");

    let announce_raw = announce_packet.to_bytes().expect("announce bytes");
    let path_response_raw = path_response_packet
        .to_bytes()
        .expect("path response bytes");
    ...
    assert!(
        report.announce_valid,
        "python failed to validate rust announce"
    );
    assert!(
        report.announce_recalled,
        "python did not cache identity from rust announce"
    );
    assert!(
        report.path_response_valid,
        "python failed to validate rust path response"
    );
    assert!(
        report.path_response_recalled,
        "python did not cache identity from rust path response"
    );
    assert_eq!(
